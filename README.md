...
Unreal script project for automatically collection of ground-truth synthetic data for segmentation tasks. The label structure obeys the ViS-WP structure.
...
- [material] Semi-automatic material assignemnt per class.
...
2. Install Datasmith Plugin for Navisworks, Sketchup, Revit or Rhino for Unreal V4.27, available in the Datasmith website [Download Datasmith](https://www.unrealengine.com/en-US/datasmith/plugins)
...
2. Enable the Python Scripting plugin from the Plugins wizard. You will be prompted to restart the engine. DO NOT RESTART IT, as we will be making more setup that will also require a project restart.
3. Configure your python paths in the project by opening the **Edit > Project Settings**, searching for Python in the search bar. Then, under the Additional Paths, add 5 new path indicating where each of the folders of this repo are located. *Note: your paths will depend on where you downloaded this repository on your computer*:
...
1. Export from modeling platform your model using the Datasmith Plugin. Depending on your arrangement of exports, you may create several `.udatasmith` files.
2. Upon relaunching your SIEProject, clean your new project from unecessary default geometry and actors. *Note: Make sure to NOT delete the `SunSky` and `ExponentialHeightFog` elements.*
   ![delete_unwanted_elems](http://url/to/img.png)
...
4. [RECOMMENDED] For better content browser organization, save all imported geometry in a separate folder in the Content Browser.
   ![SIEFolder_content_browser](http://url/to/SIEFolder_content_browser.png)
5. Once the import begins, ensure to ONLY import `Geometry`, and `Materials & Textures`. Later on, we will be creating our own `Lights`, and `Cameras`.
   ![import_options](http://url/to/import_options.png)

## Running main.py
1. Open the Python Scripting output console by enabling **Window > Developer Tool > Output Log**
2. On the console, you only need to type the name of the file `main.py` for executing it.
...
2. Run `main.py` from output console.
